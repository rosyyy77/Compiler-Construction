...
def semantic_analysis(node):
    ...
    handler_map = {
        ...
        NodeType.GLOBAL_DECL: semantic_handler_globalDecl,
        NodeType.RETURN:semantic_handler_return,
        NodeType.FUNC_DECL: semantic_handler_functionDecl,
        NodeType.VAR_DECL: semantic_handler_varDecl,
        ...
        NodeType.WHILE_LOOP: semantic_handler_for,
        NodeType.RETURN: semantic_handler_return,
...
def semantic_handler_id(node):
    if symbol_table.lookup_global(node.lexeme) is None:
        raise ValueError(f"Variable not defined: {node.lexeme}")
    else:
...
# TODO: define more hanlder functions for various node types
def semantic_handler_globalDecl(node):
    """
    detect whether the global declaration is repeated

    A global declaration should be like: global x = 5
    """
    for child in node.children:
        semantic_analysis(child)

    lexeme=child[0].lexeme
    type=child[1].datatype
    if symbol_table.lookup_global(lexeme) is None:
        symbol_table.insert(lexeme,type)
    else:
        raise ValueError(f"The global varaiable {lexeme} has already been declared.")

def semantic_handler_functionDecl(node):
    """
    detect whether the function has already been declared.
    """
    for child in node.children:
        semantic_analysis(child)

    # The program simply do not allow the same name for a function and an variable
    lexeme=child[1].lexeme
    type=child[0].datatype
    if symbol_table.lookup_global(lexeme) is None:
        symbol_table.insert(lexeme,type)
    else:
        raise ValueError(f"The global function {lexeme} has already been declared.") 

def semantic_handler_varDecl(node):
    for child in node.children:
        semantic_analysis(child)
    ...
    if symbol_table.lookup_local(lexeme) is None:
        ...
        raise ValueError(f"The local varaiable {lexeme} has already been declared.")
...
def semantic_handler_stmts(node):
    ...
    # detect the return node
    for child in node.children:
        if child.nodetype is NodeType.RETURN:
...
def semantic_handler_if(node):
    """
    detect else first, the scope should not include else-stmts
    """
    num_of_children=len(node.children)
    if node.children[num_of_children-1].nodetype == NodeType.ELSE_STMT:
        symbol_table.push_scope()
        for i in range(num_of_children-1):
            semantic_analysis(node.children[i])
        symbol_table.pop_scope()
        semantic_analysis(node.children[num_of_children-1])
    else:
        symbol_table.push_scope()
        for child in node.children:
            semantic_analysis(child)
...
def semantic_handler_else(node):
    symbol_table.push_scope()
    for child in node.children:
        semantic_analysis(child)
    symbol_table.pop_scope()
...
def semantic_handler_return(node):
    """
    The datatype of RETURN node is determined by its only child.
    """
    if(len(node.child)) != 1:
        raise ValueError("Return multiple values.")
    else:
